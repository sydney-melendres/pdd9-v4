...
if player_performance is not None:
    ...
    def prepare_event_focused_data(player_performance, mouse_keyboard_data, window_size=5):
        ...
        for _, event in player_performance.iterrows():
            ...
            if not period_data.empty:
                ...
                freq_data['Event_Type'] = 'Kill' if event['killer_ip'] == event['killer_ip'] else 'Death'
    ...
    def prepare_data_for_regression(df, selected_column):
        y = df[f'{selected_column}_diff'].astype(float)
        X = pd.get_dummies(df['Latency'], prefix='Latency', drop_first=True)
        X = sm.add_constant(X)
        X = X.astype(float)
        return X, y
    ...
    # Streamlit app
    st.title('Event-Focused Player Performance Analysis')
    ...
    event_type = st.sidebar.radio("Select Event Type", ('Kill', 'Death'))
    ...
    if selected_players and selected_latencies:
        ...
        for player in selected_players:
            ...
            player_events = player_performance[
                ((player_performance['killer_ip'] == player) & (event_type == 'Kill')) |
                ((player_performance['victim_ip'] == player) & (event_type == 'Death'))
            ]
