...
def create_frequency_data(data, columns, start_time):
    ...
    # Count events in each second for all columns
    ...
    for column in columns:
        freq_data[f'{column}_diff'] = freq_data[f'{column}_count'].diff().fillna(0)
...
for idx, col in enumerate(input_columns):
    ...
    for i, (_, change) in enumerate(latency_changes.iterrows()):
        ...
        for _, kill in period_kills.iterrows():
            ...
            if nearest_second in freq_data['normalized_time'].values:
...
plt.tight_layout()
plt.savefig('player_138.25.249.94_all_inputs_comparison_10min_no_outliers.png', dpi=300, bbox_inches='tight')
plt.close()
