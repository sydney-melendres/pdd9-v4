    ...
    for attr in attributes:
        ...
        for _, kill in kills_data.iterrows():
            ...
            if nearest_second is not None:
                kill_times.append(nearest_second)
                kill_y_values.append(freq_data.loc[freq_data['normalized_time'] == nearest_second, f'{attr}_diff'].values[0])
        ...
        fig.write_html(dir_path + f'/{attr}_frequency.html')

# Create visualizations
create_line_graphs(freq_data, kills_data)

print("Visualizations have been saved as separate HTML files for each attribute.")