...
if player_performance is not None:
    ...
    if selected_players and selected_latencies and selected_maps:
        ...
        for input_col in input_columns:
            ...
            fig.update_layout(
                title=dict(
                    text=f'{input_col.capitalize()} {y_axis_title} Between {event_type} (Time Range: {time_range[0]}s to {time_range[1]}s)',
                    font=dict(size=24)
                ),
                xaxis_title=dict(text='Player Group and Latency', font=dict(size=18)),
                yaxis_title=dict(text=y_axis_title, font=dict(size=18)),
                height=800,
                boxmode='group',
                boxgroupgap=0.2,
                boxgap=0.1,
            ...
            # Adjust y-axis to remove scaling and increase tick label size
            fig.update_layout(yaxis=dict(tickformat='.2f', tickfont=dict(size=14)))
            ...
            pct_change_fig.update_layout(
                title=dict(
                    text=f'Percentage Change in {input_col.capitalize()} Frequency from {baseline_latency}ms Latency (Time Range: {time_range[0]}s to {time_range[1]}s)',
                    font=dict(size=24)
                ),
                xaxis_title=dict(text='Latency', font=dict(size=34)),
                yaxis_title=dict(text='Percent Change', font=dict(size=34)),
                barmode='group',
                height=600,
                legend=dict(font=dict(size=34)),
                font=dict(size=34)
            )
            
            # Increase x-axis and y-axis tick label size
            pct_change_fig.update_xaxes(tickfont=dict(size=34))
            pct_change_fig.update_yaxes(tickfont=dict(size=34))
            
            st.plotly_chart(pct_change_fig, use_container_width=True, config={
                'toImageButtonOptions': {
                    'width': 2420,
                    'height': 1080,
                    'scale': 8
