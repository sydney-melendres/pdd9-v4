...
for _, kill in kills.iterrows():
    ...
    plt.scatter(kill_time, kill_window_data.loc[kill_window_data['timestamp'] == kill_time, 'deviation'].values[0], 
...
for _, death in deaths.iterrows():
    ...
    plt.scatter(death_time, death_window_data.loc[death_window_data['timestamp'] == death_time, 'deviation'].values[0], 
...
kill_window_deviations = [get_mouse_data_around_event(kill['timestamp'], time_window)['deviation'].mean() for _, kill in kills.iterrows()]
death_window_deviations = [get_mouse_data_around_event(death['timestamp'], time_window)['deviation'].mean() for _, death in deaths.iterrows()]
