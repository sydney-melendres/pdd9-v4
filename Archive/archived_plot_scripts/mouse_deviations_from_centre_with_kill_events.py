...
# Ensure the mouse data is sorted by timestamp
...
def get_nearest_mouse_data(event_time, window_data):
    if window_data.empty:
        return None
    nearest_idx = (window_data['timestamp'] - event_time).abs().idxmin()
    return window_data.loc[nearest_idx]
...
plt.savefig(dir_path + '/mouse_deviation_with_event_windows.png', dpi=300)
plt.close()
...
# Print the first few rows of each dataset to verify correct loading and processing
print("\nFirst few rows of mouse_data:")
print(mouse_data.head())
print("\nFirst few rows of player_data:")
