...
if player_performance is not None:
    ...
    def run_zero_inflated_negative_binomial(X, y):
        try:
            model = ZeroInflatedNegativeBinomialP(y, X, exog_infl=X)
            results = model.fit()
            return results
        except Exception as e:
            st.error(f"Error in Zero Inflated Negative Binomial Regression: {str(e)}")
            return None

    def run_zero_inflated_poisson(X, y):
        try:
            model = ZeroInflatedPoisson(y, X, exog_infl=X)
            results = model.fit()
            return results
        except Exception as e:
            st.error(f"Error in Zero Inflated Poisson: {str(e)}")
            return None
    ...
    show_zip = st.sidebar.checkbox('Zero Inflated Poisson Regression', value=True)
    show_zinb = st.sidebar.checkbox('Zero Inflated Negative Binomial Regression')
    ...
    if selected_players and selected_latencies:
        ...
        if all_freq_data:
            ...
            if show_zip:
                st.subheader('Zero Inflated Poisson Regression Results')
                st.write("Zero Inflated Poisson regression models count data with excess zeros and can test for the effect of latency on the rate of events.")
                
                X, y = prepare_data_for_regression(df, selected_column)
                zip_results = run_zero_inflated_poisson(X, y)
                if zip_results:
                    st.write(zip_results.summary())
                    st.write("Interpretation: The Zero Inflated Poisson model accounts for excess zeros in the data. "
                             "Look at the coefficients for both the count model and the zero inflation model. "
                             "Significant p-values (< 0.05) in the count model indicate that the corresponding latency level "
                             "has a measurable effect on the input frequency. The zero inflation model shows factors "
                             "influencing the likelihood of observing zero counts.")

            if show_zinb:
                st.subheader('Zero Inflated Negative Binomial Regression Results')
                st.write("Zero Inflated Negative Binomial regression is useful when the data is overdispersed and has excess zeros.")
                
                X, y = prepare_data_for_regression(df, selected_column)
                zinb_results = run_zero_inflated_negative_binomial(X, y)
                if zinb_results:
                    st.write(zinb_results.summary())
                    st.write("Interpretation: Similar to the Zero Inflated Poisson model, but it allows for more variability in the data. "
                            "If the results differ significantly from the ZIP model, it suggests that overdispersion "
                            "is present and the ZINB model may be more appropriate. Compare the AIC and BIC values "
                            "between ZIP and ZINB to determine which model fits the data better.")
