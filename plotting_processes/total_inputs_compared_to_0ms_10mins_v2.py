...
def plot_comparison(ax, zero_latency_data, other_latency_data, other_latency_value, kills_data, deaths_data):
    ax.plot(zero_latency_data['normalized_time'], zero_latency_data['total_inputs_diff'], 
            color='blue', label='Latency: 0', alpha=0.7)
    ax.plot(other_latency_data['normalized_time'], other_latency_data['total_inputs_diff'], 
    ...
    for _, kill in kills_data.iterrows():
        ...
        if nearest_second is not None:
            ax.plot(nearest_second, zero_latency_data.loc[zero_latency_data['normalized_time'] == nearest_second, 'total_inputs_diff'].values[0],
...
plt.savefig(dir_path + '/player_138.25.249.94_latency_comparisons_10min_no_outliers_vertical.png', dpi=300, bbox_inches='tight')
plt.close()
